...
struct cbEditorInternalData
{
    ...
    {
        const float pointSize = m_pOwner->m_pControl->StyleGetFont(wxSTC_STYLE_DEFAULT).GetPointSize();
        if (both)
        {
...
{
    m_pControl->SetFoldFlags(underline ? wxSTC_FOLDFLAG_LINEAFTER_CONTRACTED : 0);
    if (m_pControl2)
        m_pControl2->SetFoldFlags(underline ? wxSTC_FOLDFLAG_LINEAFTER_CONTRACTED : 0);
}
...
//    for (int marker = 0 ; marker <= wxSTC_MARKNUM_LASTUNUSED ; ++marker)
...
static void SetEditorTechnology(cbStyledTextCtrl *control, ConfigManager *config)
{
    ...
    if (technology == 1)
        control->SetTechnology(wxSTC_TECHNOLOGY_DIRECTWRITE);
    else
        control->SetTechnology(wxSTC_TECHNOLOGY_DEFAULT);
    ...
    default:
    case 0:
        control->SetFontQuality(wxSTC_EFF_QUALITY_DEFAULT);
        break;
    case 1:
        control->SetFontQuality(wxSTC_EFF_QUALITY_NON_ANTIALIASED);
        break;
    case 2:
        control->SetFontQuality(wxSTC_EFF_QUALITY_ANTIALIASED);
        break;
    case 3:
        control->SetFontQuality(wxSTC_EFF_QUALITY_LCD_OPTIMIZED);
        break;
    }
#else
    (void)config;
...
void cbEditor::InternalSetEditorStyleBeforeFileOpen(cbStyledTextCtrl* control)
{
    ...
    if (caretBuffer == 0)
    {
        ...
        control->SetVisiblePolicy(wxSTC_CARET_EVEN, 0); // default
    }
    else if (caretBuffer > 0 && caretBuffer <= 10)
    {
        // margin of N lines at top/bottom
        control->SetYCaretPolicy(wxSTC_CARET_SLOP | wxSTC_CARET_STRICT | wxSTC_CARET_EVEN,
                                 caretBuffer);
        control->SetVisiblePolicy(wxSTC_CARET_SLOP | wxSTC_CARET_STRICT | wxSTC_CARET_EVEN,
                                  caretBuffer);
    }
    else
    {
        ...
        control->SetVisiblePolicy(wxSTC_CARET_STRICT | wxSTC_CARET_EVEN, 4);
    }
...
void cbEditor::InternalSetEditorStyleAfterFileOpen(cbStyledTextCtrl* control)
{
    ...
    if (mgr->ReadBool(_T("/folding/show_folds"), true))
    {
        ...
        control->SetFoldFlags(wxSTC_FOLDFLAG_LINEAFTER_CONTRACTED);
...
void cbEditor::DoFoldAll(FoldMode fold)
{
    ...
    if (fold != FoldMode::toggle)
    {
        ...
        for (int line = 0; line < count; ++line)
        {
            ...
            if (m_pData->mFoldingLimit)
            {
                ...
                {
                    int levelNumber = (level & wxSTC_FOLDLEVELNUMBERMASK) - wxSTC_FOLDLEVELBASE;
    ...
    else
    {
        ...
        for (int line = 0; line < count; ++line)
        {
            ...
            if ((level & wxSTC_FOLDLEVELHEADERFLAG) == 0)
                continue;
            const int levelNumber = (level & wxSTC_FOLDLEVELNUMBERMASK) - wxSTC_FOLDLEVELBASE;
            if (m_pData->mFoldingLimit)
            {
...
void cbEditor::DoFoldBlockFromLine(int line, FoldMode fold, unsigned foldFlags)
{
    ...
    static_assert(int(cbEditor::FoldMode::contract) == wxSTC_FOLDACTION_CONTRACT, "Must match");
    static_assert(int(cbEditor::FoldMode::expand) == wxSTC_FOLDACTION_EXPAND, "Must match");
    static_assert(int(cbEditor::FoldMode::toggle) == wxSTC_FOLDACTION_TOGGLE, "Must match");
...
//    if (flags & wxSTC_PERFORMED_USER) txt << _T("wxSTC_PERFORMED_USER, ");
